...
// Animated number component — smooth count-up/down on value change
function AnimNum({value,color="#fff",size=14,weight=700}){
  ...
  return <span style={{fontSize:size,fontWeight:weight,color,fontFamily:"inherit"}}>{fmt(display)}</span>;
}
    ...
    <div style={{fontFamily:"'SF Mono',monospace",background:bg,color:txt,minHeight:"100vh",padding:12}} onClick={()=>setOpenDrop(null)}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8,borderBottom:`1px solid ${brd}`,paddingBottom:6}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontSize:8,color:"#334155"}}>v3.3 · ISQ 2024</span>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:4,marginBottom:8}}>
        {[{k:"t",l:"TOTAL",i:"◉"},{k:"tms",l:"TMS",i:"⚡"},{k:"psy",l:"PSYCHO",i:"◈"},{k:"mac",l:"MACHINE",i:"⚙"},{k:"sur",l:"SURDITÉ",i:"◎"},{k:"cov",l:"COVID",i:"◆"}].map(({k,l,i})=>(
          <button key={k} onClick={()=>setInd(k)} style={{background:ind===k?`${IND[k].c}12`:card,border:`1px solid ${ind===k?IND[k].c:brd}`,borderRadius:5,padding:"5px 3px",cursor:"pointer",textAlign:"left",fontFamily:"inherit"}}>
            ...
            <AnimNum value={yd[k]} color={ind===k?"#fff":"#94a3b8"} size={14} weight={700}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:hasRight?"1fr 280px":"1fr",gap:8}}>
        <div style={{background:card,border:`1px solid ${brd}`,borderRadius:7,padding:10,minHeight:320,position:"relative",overflow:"hidden"}}>
          {view==="sec"&&<div key={`sec-${year}-${ind}`}>
            ...
            {secBars.map(([name,val],i)=>(
              <div key={name} style={{display:"flex",alignItems:"center",gap:5,marginBottom:2,animation:`barSlide 0.4s ease ${i*50}ms both`}}>
                ...
                <div style={{flex:1,height:14,background:"#0d1525",borderRadius:3,overflow:"hidden"}}>
                  <div style={{height:"100%",width:`${(val/maxSec)*100}%`,background:`linear-gradient(90deg,${IND[ind].c}88,${IND[ind].c})`,borderRadius:3,transition:"width 0.6s cubic-bezier(0.34,1.56,0.64,1)"}}/>
                </div>
          ...
          {(view==="map"||view==="cross")&&<>
            ...
            <svg viewBox={`0 0 ${MW} ${MH}`} style={{width:"100%",height:"auto",maxHeight:400,display:"block"}}>
              ...
              {bubbles.map(b=><circle key={`g${b.c}`} cx={b.x} cy={b.y} r={b.radius*1.5} fill={b.color} opacity={0.06} filter="url(#bl)" style={{transition:"r 0.6s ease, fill 0.6s ease"}}/>)}
              ...
              {bubbles.map(b=>{const sel=selReg===b.c,hov=hovReg===b.c;return<g key={b.c} onClick={()=>setSelReg(selReg===b.c?null:b.c)} onMouseEnter={()=>setHovReg(b.c)} onMouseLeave={()=>setHovReg(null)} style={{cursor:"pointer"}}>
                ...
                <circle cx={b.x} cy={b.y} r={hov?b.radius+2:b.radius} fill={b.color} fillOpacity={sel?0.9:0.6} stroke={sel?"#00ff88":hov?"#fff":b.color} strokeWidth={sel?2:hov?1.5:0.5} className="bubble-morph"/>
                <text x={b.x} y={b.y-1} textAnchor="middle" fill="#fff" fontSize={b.radius>16?8:6.5} fontWeight="600" style={{pointerEvents:"none",transition:"font-size 0.4s"}}>{b.s}</text>
                <text x={b.x} y={b.y+(b.radius>16?8:6.5)} textAnchor="middle" fill="#ffffffbb" fontSize={b.radius>14?6.5:5.5} style={{pointerEvents:"none",transition:"font-size 0.4s"}}>{fmt(b.val)}</text>
              </g>})}
        ...
        {hasRight&&<div style={{background:card,border:`1px solid ${brd}`,borderRadius:7,padding:8,overflowY:"auto",maxHeight:400}}>
          {view==="map"&&regDetail&&<>
            ...
            <div style={{background:"#0d1525",borderRadius:6,padding:6,marginBottom:8,textAlign:"center"}}>
              <div style={{fontSize:22,fontWeight:800,color:"#60a5fa"}}><AnimNum value={regDetail.val} color="#60a5fa" size={22} weight={800}/></div>
          ...
          {view==="sec"&&filter&&filterEvo&&<>
            ...
            <div style={{background:"#0d1525",borderRadius:6,padding:6,marginTop:8,textAlign:"center"}}>
              <div style={{fontSize:20,fontWeight:800,color:DIMS[filter.dim]?.c||"#60a5fa"}}><AnimNum value={YD[filter.dim]?.[String(year)]?.[filter.val]||0} color={DIMS[filter.dim]?.c||"#60a5fa"} size={20} weight={800}/></div>
      ...
      <style>{`
        ...
        @keyframes barSlide{from{opacity:0;transform:translateX(-12px)}to{opacity:1;transform:translateX(0)}}
        .bubble-morph{transition:r 0.6s cubic-bezier(0.34,1.56,0.64,1),fill 0.5s ease,fill-opacity 0.3s ease,stroke 0.3s ease,stroke-width 0.2s ease}
      `}</style>
    </div>
  );
}